...
def analyze_ticker(ticker):
    try:
        ...
        above_200 = "N/A" if ma200_val is None else ("✅ Above" if current > ma200_val else "❌ Below")

        iv = get_iv_30d(ticker)
        ...
        return {
            ...
            "200W MA":         above_200,
            "IV (30D)":        iv,
            "Weight%":         "N/A",  # filled in later for QQQ/SPMO
        }
...
def get_etf_holdings(ticker, name):
    """Scrape holdings and weights from stockanalysis.com."""
    print(f"Fetching {name} holdings...")
    try:
        tables = pd.read_html(f"https://stockanalysis.com/etf/{ticker.lower()}/holdings/")
        for t in tables:
            cols = [str(c).lower() for c in t.columns]
            if any("symbol" in c or "ticker" in c for c in cols):
                sym_col = [c for c in t.columns if "symbol" in str(c).lower() or "ticker" in str(c).lower()][0]
                wgt_col = next((c for c in t.columns if "weight" in str(c).lower() or "%" in str(c)), None)
                tickers = [str(x).strip().upper() for x in t[sym_col].dropna() if str(x).strip() not in ["", "nan"]]
                weights = {}
                if wgt_col is not None:
                    for _, row in t.iterrows():
                        sym = str(row[sym_col]).strip().upper()
                        try:
                            w = str(row[wgt_col]).replace("%","").strip()
                            weights[sym] = f"{float(w):.2f}%"
                        except:
                            weights[sym] = "N/A"
                if tickers:
                    print(f"  Found {len(tickers)} {name} holdings with weights")
                    return tickers, weights
    except Exception as e:
        print(f"  Scrape failed: {e}")

    # Fallback — no weights available
    print(f"  Using hardcoded {name} fallback (no weights)")
    if ticker.upper() == "QQQ":
        fallback = [
...
def read_tickers_from_sheet(wb_bytes, sheet_name, seed_tickers, data_start_row=6):
    try:
        ...
        wb.close()
        return tickers if tickers else seed_tickers
    except:
        return seed_tickers
...
def get_styles():
    return {
        ...
        "weight_font":    Font(name="Calibri", size=11, color="2C3E50"),
        "iv_font":        Font(name="Calibri", size=11, color="2C3E50"),
...
def _write_header(ws, title, generated_at, col_letter, has_instruction=False):
    ...
    ws["A2"].value     = (f"  Generated: {generated_at}   |   MACD (12,26,9)   |   "
                          f"RSI({RSI_PERIOD}) vs MA({RSI_MA})   |   200W SMA   |   IV: ATM 30D   |   Weekly   |   Source: Yahoo Finance")
...
def _write_data(ws, results, header_row, headers, col_widths,
                has_sector=False, has_weight=False, preserve_order=False):
    s          = get_styles()
    thick_b    = Border(left=s["med"],  right=s["med"],  top=s["med"],  bottom=s["med"])
    thin_b     = Border(left=s["thin"], right=s["thin"], top=s["thin"], bottom=s["thin"])
    ...
    for row_idx, r in enumerate(ordered, header_row + 1):
        ...
        row_data = [(r["Ticker"], tf, s["ticker_font"])]

        if has_weight:
            row_data.append((r.get("Weight%", "N/A"), base_fill, s["weight_font"]))
        if has_sector:
            row_data.append((r.get("Sector", ""), base_fill, s["sector_font"]))
        ...
        row_data += [
            ...
            (r.get("IV (30D)", "N/A"), base_fill,        s["iv_font"]),
        ]
...
def build_sheet(ws, results, title, generated_at,
                has_sector=False, has_weight=False):
    ws.title = title
    headers, col_widths = [], []

    headers.append("Ticker");     col_widths.append(12)
    if has_weight:
        headers.append("Weight%"); col_widths.append(10)
    if has_sector:
        headers.append("Sector");  col_widths.append(26)
    headers += ["MACD Cross Date","MACD Signal","RSI Cross Date","RSI Signal","200W MA","IV (30D)"]
    col_widths += [20, 18, 20, 18, 14, 10]

    col_letter = get_column_letter(len(headers))
    header_row = _write_header(ws, title, generated_at, col_letter)
    _write_data(ws, results, header_row, headers, col_widths,
                has_sector=has_sector, has_weight=has_weight)

def build_editable_sheet(ws, results, title, generated_at):
    ws.title = title
    headers    = ["Ticker","MACD Cross Date","MACD Signal","RSI Cross Date","RSI Signal","200W MA","IV (30D)"]
    col_widths = [14, 20, 18, 20, 18, 14, 10]
    col_letter = get_column_letter(len(headers))
...
def upload_to_drive(service, wb, file_id):
    ...
    if not file_id or file_id.lower() == "placeholder":
        ...
        new_file = service.files().create(
            body={"name": "QQQ_MACD_RSI_Crossovers.xlsx"},
            media_body=media, fields="id"
        ).execute()
        new_id = new_file.get("id")
        print(f"✅ Created new file (ID: {new_id})")
        print(f"⚠️  Update your GDRIVE_FILE_ID secret to: {new_id}")
    else:
        try:
            media = MediaInMemoryUpload(content, mimetype=mime, resumable=False)
            service.files().update(fileId=file_id, media_body=media).execute()
            print(f"✅ Updated file in Google Drive (ID: {file_id})")
        except Exception as e:
            print(f"  Update failed ({e}) — creating new file...")
            media    = MediaInMemoryUpload(content, mimetype=mime, resumable=False)
            new_file = service.files().create(
                body={"name": "QQQ_MACD_RSI_Crossovers.xlsx"},
                media_body=media, fields="id"
...
if __name__ == "__main__":
    ...
    print("\nReading editable tickers...")
    watchlist_tickers  = read_tickers_from_sheet(existing_bytes, "Watchlist",  WATCHLIST_SEED,  6) if existing_bytes else WATCHLIST_SEED
    financials_tickers = read_tickers_from_sheet(existing_bytes, "Financials", FINANCIALS_SEED, 6) if existing_bytes else FINANCIALS_SEED

    # Get ETF holdings + weights
    qqq_tickers,  qqq_weights  = get_etf_holdings("QQQ",  "QQQ")
    spmo_tickers, spmo_weights = get_etf_holdings("SPMO", "SPMO")
    ...
    # Inject weights into QQQ and SPMO results
    for r in qqq_results:
        r["Weight%"] = qqq_weights.get(r["Ticker"], "N/A")
    for r in spmo_results:
        r["Weight%"] = spmo_weights.get(r["Ticker"], "N/A")

    # Build workbook
    generated_at = datetime.now().strftime("%B %d, %Y  %I:%M %p UTC")
    wb = Workbook()
    build_sheet(wb.active,         qqq_results,       "QQQ Holdings", generated_at, has_weight=True)
    ...
    build_sheet(wb.create_sheet(), spmo_results,       "SPMO",         generated_at, has_weight=True)
